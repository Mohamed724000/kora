...
| DEC-M0.3-11 | Documentation  | M0.3-R3 réconcilie localement les quatre documents vivants M0.3 avec l’état R2 publié. Aucun code, manifeste, lockfile, scanner, workflow ou état GitHub n’est modifié ; un commit reste soumis à une décision CTO distincte.                                      | Décision CTO M0.3-R3                   | Executed locally — pending commit decision |

La séquence est cumulative : M0.3 corrige `deepmerge-ts`, R1 durcit le gate et
R2 corrige `mysql2` sans changer Prisma `7.9.1`. Les anciennes quantités de
validation établies avant R2 sont remplacées par DEC-M0.3-09 pour l’état final
applicable. S1.1 reste préservé à 39/39 avec l’empreinte agrégée
